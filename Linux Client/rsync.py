...
def sync():
	import requests, os,sys,sqlite3,hashlib,json,signal
	import tqdm, time, base64 
	...
	def uploadfile(path):
		...
		try:
			...
			data = dict(path = path, sha256 = sha256, docfile=docfile , isdir=isdir, schema=lcschema,csrfmiddlewaretoken=csrftoken,  next='')
	...
	if g['possible']=='True':
		...
		if dbschema!=lcschema:
			...
			if decision=='continue':
				filelist = [ f for f in os.listdir(observing_root) if f.endswith(".bak") ]
				for f in filelist:
					os.remove(os.path.join(observing_root, f))
				list_of_lcfiles=[]
				dict_of_lcfiles={}
				...
				if dbschema=='AES':
					...
					specific = "-des-cbc"
				elif dbschema== 'DES-EDE3':
					specific = "-des-ede3-cbc"
				else:
					print("Unknown schema")

				bash_command = ' openssl enc -nosalt -base64 -P ' + specific + ' > metadata.txt'
				os.system(bash_command)
				bs_2 =  "echo 'enc='" + str(dbschema) + " >> metadata.txt"
				# print(bs_2)
				os.system(bs_2)
