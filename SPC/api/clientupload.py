...
ROOT = '/home/jatin/iitb/third_sem/cs251/project_test_folder'
for root, dirs, files in os.walk(ROOT):
    ...
    for fpath in [osp.join(root, f) for f in files]:
        ...
        file = open(fpath, 'rb')
        owner = 'jatin'
        data_dict = dict(path = name, sha256 = sha, docfile=file, owner=owner)   #csrfmiddlewaretoken=csrftoken, next='/')
        r = client.post(URL, data=data_dict, headers=dict(Referer=URL))
        print (sha, name)
